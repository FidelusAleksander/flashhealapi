    ...
    def get(self):
        try:
            reviews = db.session.query(DoctorReviewModel.doctor_id,
                                       ...
                                       db.func.count(DoctorReviewModel.star_rating).label('reviews_count'),
        ...
        summaries = [self.combine(result.DoctorModel, result.avg_review, result.reviews_count) for result in results]

        return summaries, 200



    def combine(self, doctor, avg_review, reviews_count):
        summary = doctor.json()
        summary['avg_review'] = avg_review if avg_review else 0
        summary['reviews_count'] = reviews_count if reviews_count else 0
        return summary
